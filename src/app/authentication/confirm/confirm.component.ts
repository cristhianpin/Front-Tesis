...
export class ConfirmComponent implements OnInit, OnDestroy {
  ...
  public confirmBtn = 'Nueva contraseña';
  ...
  private initValues(): void {
    ...
    // get data url
    this.activatedRoute.params.subscribe((params) => {
      //this.logger.debug('confirm-account, params: ', params);
    ...
    // validate params
    this.authService.confirmUser(this.userId, this.tokenConfirm).subscribe((res: any) => {
      if (res) {
        //this.logger.debug('confirmUser, res: ', res);
  ...
  public onSubmitNewToken(): void {
    ...
    const newToken$ = this.authService.newToken(this.obj).subscribe((res: any) => {
      if (res) {
        //this.logger.debug('newToken, res: ', res);
        if (res.code == 200 || res.code == 201) {
          if (res.data.messageError) {
            this.showLogin = true;
