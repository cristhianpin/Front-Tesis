...
@Component({
  selector: 'ngx-approval-payment-list',
  templateUrl: './approval-payment-list.component.html',
  styleUrls: ['./approval-payment-list.component.scss'],
})
export class ApprovalPaymentListComponent extends BaseListComponent implements OnInit, OnDestroy {
  ...
  public handleRowSelect(event: any): void {
    // Modificar event.data.id para que use otra propiedad
  ...
  private findAll(): void {
    ...
    const dataList$ = this.statementAccountService.payment('APROBADO').subscribe((res: any) => {
      if (res) {
        this.objs = res.data;
